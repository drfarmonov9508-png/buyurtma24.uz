...
export const billiardApi = {
  ...
  getTypes: (clubId: string) => api.get(`/v1/billiard/clubs/${clubId}/types`),
  getExtras: (clubId: string) => api.get(`/v1/billiard/clubs/${clubId}/extras`),
  bookTable: (tableId: string, data?: { note?: string }) => api.post(`/v1/billiard/tables/${tableId}/book`, data || {}),
  requestOrderItem: (orderId: string, data: { extraId: string; quantity?: number }) =>
    api.post(`/v1/billiard/orders/${orderId}/request-item`, data),
  getMyOrders: () => api.get('/v1/billiard/orders/my'),
  adminSnapshot: () => api.get('/v1/billiard/admin/snapshot'),
  getAnalytics: (params?: any) => api.get('/v1/billiard/admin/analytics', { params }),
  createType: (data: any) => api.post('/v1/billiard/types', data),
  createTable: (data: any) => api.post('/v1/billiard/tables', data),
  getTableByQr: (token: string) => api.get(`/v1/billiard/qr/${token}`),
  bookTableByQr: (token: string, phone: string) => api.post(`/v1/billiard/qr/${token}/book`, { phone }),
  updateTable: (id: string, data: any) => api.patch(`/v1/billiard/tables/${id}`, data),
  deleteTable: (id: string) => api.patch(`/v1/billiard/tables/${id}/delete`, {}),
  deleteExtra: (id: string) => api.patch(`/v1/billiard/extras/${id}/delete`, {}),
  createExtra: (data: any) => api.post('/v1/billiard/extras', data),
