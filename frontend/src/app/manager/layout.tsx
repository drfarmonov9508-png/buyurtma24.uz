...
import {
  LayoutDashboard, Package, Tag, Grid3X3, Users,
  LogOut, ChevronLeft, ChevronRight, UtensilsCrossed, Menu, ClipboardList
} from 'lucide-react';

const NAV = [
  { href: '/manager', label: 'Bosh sahifa', icon: LayoutDashboard, exact: true },
  { href: '/manager/orders', label: 'Buyurtmalar', icon: ClipboardList },
  { href: '/manager/products', label: 'Mahsulotlar', icon: Package },
  { href: '/manager/categories', label: 'Kategoriyalar', icon: Tag },
  { href: '/manager/tables', label: 'Stollar', icon: Grid3X3 },
  { href: '/manager/staff', label: 'Xodimlar', icon: Users },
];
        ...
        <div className="flex items-center justify-between h-16 px-4 border-b border-gray-100 dark:border-gray-800">
          {!collapsed && (
            <div className="flex items-center gap-2.5">
              <div className="w-8 h-8 bg-gradient-to-br from-indigo-500 to-indigo-700 rounded-xl flex items-center justify-center shadow-lg shadow-indigo-500/25">
                <UtensilsCrossed className="w-4 h-4 text-white" />
              </div>
              <div>
                <p className="font-bold text-gray-900 dark:text-white text-sm leading-none">Buyurtma24</p>
                <p className="text-[10px] text-indigo-500 font-medium">Ish yurituvchi</p>
              </div>
            </div>
          )}
          <button
